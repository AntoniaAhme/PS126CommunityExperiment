...
# Export ASV + tax table for species trait grouping
asv.srs_2 <- asv.srs
asv.srs_2$ASV <- rownames(asv.srs_2)
writexl::write_xlsx(asv.srs_2, "Data/ASVsrs.xlsx")
writexl::write_xlsx(TAXasv, "Data/TAXasv.xlsx")
...
sam$RichnessE <- ps.rich$Observed
sam$EvennessE <- ps.rich$Evenness

# Add shannon diversity to t-fin subset of sample tab
sam_tfin <- subset(sam, timepoint == "tfin")
row.names(sam_tfin) <- NULL

# Add richness & evenness of prokaryotes (calculated in other script)
sam_tfin$RichnessP <- c(185, 181, 197, 160, 184, 171, 201, 203, 199)
sam_tfin$EvennessP <- c(0.431275329, 0.561688848, 0.550855022, 0.495907533, 
                        0.502801915, 0.424280739, 0.554418443, 0.564937512, 0.539966221)
...
class.leg
ggsave("Output/Figure#2ClassLegend.png", class.leg, height = 8, width = 4, dpi = 320)
...
class_plot
ggsave("Output/Figure#2Classes.png", class_plot, height = 5, width = 5, dpi = 320)
...
dia.leg
ggsave("Output/Figure#2DiaLegend.png", dia.leg, height = 6, width = 4, dpi = 320)
...
diatoms_plot
ggsave("Output/Figure#2Diatoms.png", diatoms_plot, height = 5, width = 7, dpi = 320)
...
prym.leg
ggsave("Output/Figure#2PrymLegend.png", prym.leg, height = 3, width = 4, dpi = 320)
...
prym_plot
ggsave("Output/Figure#2Prymnesiophytes.png", prym_plot, height = 5, width = 7, dpi = 320)
...
### SIZE 
## Prepare data
size_fin <- read.table("Data/Size.txt", header=FALSE)
group <- size_fin
group <- t(group)
...
#69.569086     22.950426      4.371391      3.109096 
...
## CA biplot of sizes
png(file="Output/Figure#3a.png",width=6,height=4.5,units="in",res=144)
rescale <- 0.4
dim <- c(1,2)
perc.hor <- 93.665; perc.ver <- 5.573
...
### Trophy
## Prepare data
trophy_fin <- read.table("Data/Trophy.txt", header=FALSE)
group <- trophy_fin
group <- t(group)
group <- as.data.frame(group)
colnames(group) <- group[1,]
...
#33.930796     28.232355     20.911539     14.681101      2.244209  
...
png(file="Output/Figure#3b.png",width=6,height=4.5,units="in",res=144)
rescale <- 0.4
dim <- c(1,2)
perc.hor <- 90.802; perc.ver <- 8.074
par(mar=c(4.2,4,2,2.5), mgp=c(2,0.7,0), font.lab=2, cex.axis=0.8)
plot(1.05 * rbind(tgroup.ca.cpc, rescale*tgroup.ca.rcc), type = "n", asp = 1, 
     xlab = paste("CA dimension ", dim[1], " (", round(perc.hor, 1), "%)", sep = ""), 
     ylab = paste("CA dimension ", dim[2], " (", round(perc.ver, 1), "%)", sep = ""), 
...
### Thermal niche
## Prepare data
therm_fin <- read.table("Data/Therm.txt", header=FALSE)
group <- therm_fin
group <- t(group)
group <- as.data.frame(group)
colnames(group) <- group[1,]
...
#55.088928        27.266371        13.253819         4.390883 
...
png(file="Output/Figure#3c.png",width=6,height=4.5,units="in",res=144)
rescale <- 0.4
dim <- c(1,2)
perc.hor <- 89.057; perc.ver <- 7.322
par(mar=c(4.2,4,2,2.5), mgp=c(2,0.7,0), font.lab=2, cex.axis=0.8)
plot(1.05 * rbind(tgroup.ca.cpc, rescale*tgroup.ca.rcc), type = "n", asp = 1, 
     xlab = paste("CA dimension ", dim[1], " (", round(perc.hor, 1), "%)", sep = ""), 
     ylab = paste("CA dimension ", dim[2], " (", round(perc.ver, 1), "%)", sep = ""), 
...
meta.wf <-rename(meta.wf, "Chla:POC" = "Chla.POC")

meta_sig <- c('temperature','Chla:POC', 'D2', 'C:N', 'RichnessE', 'EvennessE', 'RichnessP', 'EvennessP')
...
RDA_plot_cons <- ggplot(data=sites.long, 
    ...
    geom_segment(data=subset(vectors.long, vector %in% c('Chla:POC', 'C:N', 'D2', 'RichnessE', 'EvennessE', 'RichnessP', 'EvennessP')),
                 aes(x=0, y=0, xend=axis1*3, yend=axis2*3), 
                 lineend = 'round', linejoin = 'bevel',
                 colour="black", size=0.705, arrow=arrow(length = unit(0.2,"cm"))) +
    geom_text_repel(data=subset(vectors.long, vector %in% c('Chla:POC', 'C:N', 'D2','RichnessE', 'EvennessE', 'RichnessP', 'EvennessP')), 
...
RDA_plot_cons
ggsave("Output/Figure#5.png", RDA_plot_cons, height = 6, width = 5.3, dpi = 320)
...
# Chla:POC
fun$ChlC <- log(fun$ChlC)
chlc <- fun %>%
  pairwise_t_test(ChlC ~ temperature, p.adjust = "bonferroni")
# Compare to one-way ANOVA with post-hoc TUKEY t-tests
res.aov <- aov(ChlC ~ temperature, data = fun)
summary(res.aov)
TukeyHSD(res.aov)
# Results do not deviate from pairwise a priori t-tests
...
# rich E
fun$RichnessE <- log(fun$RichnessE)
richE <- fun %>%
  pairwise_t_test(RichnessE ~ temperature, p.adjust = "bonferroni")
# Compare to one-way ANOVA with post-hoc TUKEY t-tests
res.aov <- aov(RichnessE ~ temperature, data = fun)
summary(res.aov)
TukeyHSD(res.aov)
# Results do not deviate from pairwise a priori t-tests

# even E
fun$EvennessE <- log(fun$EvennessE)
evenE <- fun %>%
  pairwise_t_test(EvennessE ~ temperature, p.adjust = "bonferroni")
# Compare to one-way ANOVA with post-hoc TUKEY t-tests
res.aov <- aov(EvennessE ~ temperature, data = fun)
summary(res.aov)
TukeyHSD(res.aov)
# Results do not deviate from pairwise a priori t-tests

# rich P
fun$RichnessP <- log(fun$RichnessP)
richP <- fun %>%
  pairwise_t_test(RichnessP ~ temperature, p.adjust = "bonferroni")
# Compare to one-way ANOVA with post-hoc TUKEY t-tests
res.aov <- aov(RichnessP ~ temperature, data = fun)
summary(res.aov)
TukeyHSD(res.aov)
# Results do not deviate from pairwise a priori t-tests

# even P
fun$EvennessP <- log(fun$EvennessP)
evenP <- fun %>%
  pairwise_t_test(EvennessP ~ temperature, p.adjust = "bonferroni")
# Compare to one-way ANOVA with post-hoc TUKEY t-tests
res.aov <- aov(EvennessP ~ temperature, data = fun)
summary(res.aov)
TukeyHSD(res.aov)
# Results do not deviate from pairwise a priori t-tests
...
sig$si <- si$p.adj
sig$d2 <- d2$p.adj
sig$richE <- richE$p.adj
sig$evenE <- evenE$p.adj
sig$richP <- richP$p.adj
sig$evenP <- evenP$p.adj

require(writexl)
write_xlsx(sig, "Data/Significances.xlsx")
